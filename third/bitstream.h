...
struct ones {
    enum {
        value = 1 << (WIDTH - 1) | ones<WIDTH - 1>::value
    };
};
...
class Bitstream {
public:
    explicit Bitstream(uint32_t length) : mLength(length), mOffset(0) {}
...
public:
    BitstreamReader(const uint8_t *data, uint32_t length)
            : Bitstream(length), mData(data) {}

    ~BitstreamReader() override = default;
     ...
     * construct a value of the appropriate type for it
     *
    ...
    typename GetWidthType<N>::Type getWithOffset(size_t offset) const {
        ...
        mOffset = offset;
        auto *thisM = const_cast<BitstreamReader *>(this);
...
public:
    ...
    ~BitstreamWriter() override = default;
