...
namespace {
    ...
    sample resample(URng& rng, unsigned int resamples, std::vector<double>::iterator first,
...
#pragma warning(pop)
            ...
            if (bazelOutputFilePtr != nullptr) {
                m_data.reporterSpecifications.push_back({"junit", std::string(bazelOutputFilePtr), {}, {}});
            }
        }
...
namespace Catch {
    ...
    ScopedMessage::ScopedMessage(ScopedMessage&& old) noexcept : m_info(CATCH_MOVE(old.m_info)) { old.m_moved = true; }
...
namespace Catch {
    ...
    namespace {
        ...
        kvPair splitKVPair(StringRef kvString) {
            auto splitPos =
                static_cast<size_t>(std::distance(kvString.begin(), std::find(kvString.begin(), kvString.end(), '=')));
...
namespace Catch {
        ...
        struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker {
            ...
            static GeneratorTracker& acquire(TrackerContext& ctx,
                ...
                if (currentTracker.nameAndLocation() == nameAndLocation) {
                    ...
                    auto newTracker =
                        Catch::Detail::make_unique<GeneratorTracker>(nameAndLocation, ctx, &currentTracker);
    ...
    void RunContext::handleMessage(AssertionInfo const& info, ResultWas::OfType resultType, StringRef message,
                                   AssertionReaction& reaction) {
        m_reporter->assertionStarting(info);
...
namespace Catch {
    ...
    Section::Section(SectionInfo&& info)
        : m_info(CATCH_MOVE(info)), m_sectionIncluded(getResultCapture().sectionStarted(m_info, m_assertions)) {
    ...
    namespace TestCaseTracking {
        ...
        SectionTracker& SectionTracker::acquire(TrackerContext& ctx, NameAndLocation const& nameAndLocation) {
            ...
            if (ITracker* childTracker = currentTracker.findChild(nameAndLocation)) {
                ...
                auto newSection = Catch::Detail::make_unique<SectionTracker>(nameAndLocation, ctx, &currentTracker);
...
namespace Catch {
    ...
    XmlWriter& XmlWriter::writeText(StringRef text, XmlFormatting fmt) {
        CATCH_ENFORCE(!m_tags.empty(), "Cannot writeDataBlocks text as top level element");
        if (!text.empty()) {
...
namespace Catch {
    ...
    namespace Matchers {
        ...
        WithinUlpsMatcher::WithinUlpsMatcher(double target, uint64_t ulps, Detail::FloatingPointKind baseType)
            ...
            CATCH_ENFORCE(
                m_type == Detail::FloatingPointKind::Double || m_ulps < (std::numeric_limits<uint32_t>::max)(),
...
namespace Catch {
    ...
    struct SummaryColumn {
        SummaryColumn(std::string _label, Colour::Code _colour) : label(CATCH_MOVE(_label)), colour(_colour) {}
    ...
    void ConsoleReporter::printSummaryRow(StringRef label, std::vector<SummaryColumn> const& cols, std::size_t row) {
        for (auto col : cols) {
            ...
            if (col.label.empty()) {
                ...
                m_stream << m_colour->guardColour(Colour::LightGrey) << " | " << m_colour->guardColour(col.colour)
                         << value << ' ' << col.label;
            }
        }
        m_stream << '\n';
